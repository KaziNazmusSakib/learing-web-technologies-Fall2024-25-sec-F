<?php
session_start();
require_once('../Model/empModel.php'); // Assuming empModel.php handles database operations
...
if (isset($_COOKIE['flag'])) {
    ...
    <head>
        ...
        <script>
            function validateForm() {
                const empname = document.forms['updateForm']['empname'].value.trim();
                const contactNo = document.forms['updateForm']['contact_no'].value.trim();
    ...
    <body>
        <form name="updateForm" method="post" action="update.php" onsubmit="return validateForm()">
            ...
            <input type="text" id="empname" name="empname" value="<?php echo isset($_POST['empname']) ? $_POST['empname'] : ''; ?>"> <br>
    ...
    if (isset($_POST['submit'])) {
        $id = isset($_POST['id']) ? $_POST['id'] : (isset($_SESSION['id']) ? $_SESSION['id'] : null);

        // Check if the required fields are set and not empty
        $empname = isset($_POST['empname']) ? trim($_POST['empname']) : '';
        $contact_no = isset($_POST['contact_no']) ? trim($_POST['contact_no']) : '';
        $username = isset($_POST['username']) ? trim($_POST['username']) : '';
        $password = isset($_POST['password']) ? trim($_POST['password']) : '';

        // Validate fields on the server side as a fallback
